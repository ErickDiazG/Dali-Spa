    ...
    <header className={`fixed top-0 inset-x-0 z-50 transition-all duration-300 ${scrolled ? "bg-[var(--cream)] shadow-md" : "bg-black/70 backdrop-blur-sm"}`}>
      ...
      <div className="relative z-20 border-b border-white/10">
        <div className="max-w-7xl mx-auto px-4 flex justify-between items-center h-14 md:h-16">
          <div className="flex gap-5 items-center text-[11px] tracking-wider ml-2 md:ml-0">
            <a 
              href={`https://wa.me/${siteInfo.phoneRaw.replace('+', '').replace(/\s/g, '')}?text=${encodeURIComponent("Hola Dali Spa! 👋 Vengo del sitio web y tengo una duda sobre cómo agendar mis servicios.")}`} 
              target="_blank" 
              rel="noreferrer" 
              className={`flex items-center gap-1.5 hover:text-burgundy transition-colors ${txm}`}
            >
              <WhatsAppIcon size={14} /> 
              <span className="hidden md:inline">{siteInfo.phone}</span>
            </a>
          </div>
  ...
  return (
    <section className="bg-navy py-20 px-4">
      <div className="max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-12 lg:gap-24 items-center">
        ...
        {/* Right Side: Info */}
        <Reveal className="space-y-10 text-white text-center md:text-left flex flex-col items-center md:items-start">
          <div className="space-y-4">
            <h2 className="text-3xl lg:text-4xl font-serif tracking-wider text-cream">{t('contactInfo.title')}</h2>
            <div className="w-12 h-0.5 bg-burgundy mx-auto md:mx-0" />
          </div>
          ...
          <div className="space-y-8 w-full">
            ...
            <div className="space-y-3">
              ...
              <p className="text-sm font-sans tracking-wide leading-relaxed font-light text-white/90 mb-4 flex items-center justify-center md:justify-start gap-2">
